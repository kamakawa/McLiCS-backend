...
// Construtor: Inicializa variaveis locais e imprime parametros do campo eletrico
...
int electricEvolveN::run() {
  float S1, S2, sTemp;
  float tempE, E2, E;
  float vec_nt[3]; // Variavel auxiliar
  float vec_n[3];
  float mat_n[9];
  float ang_var = 0.5;
  char fname[1000];
  ...
  // Define direcao do loop (crescente ou decrescente)
  ...
  // Loop principal de variacao do Campo Eletrico
  for (params->elecE = params->elecEi; 
       (int)(1e6 * sign * (params->elecE - params->elecEf)) >= 0; 
