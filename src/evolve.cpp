...
// Calculadora de Energia do sistema
...
#pragma omp parallel for simd reduction(+ : Etot) schedule(simd : dynamic) num_threads(omp_get_max_threads())
  for (int idx = 0; idx < Nt; idx++) {
    ...
    // Verificacao de limites (Error handling)
    if (i >= Nx || i < 0) {
    ...
    // Vizinhos de primeira ordem (Alinhados para leitura facil)
    ...
    // Tratamento de pontos especiais
    if (nLocal[0].pt > 1) {
...
void EvolveN::Monte_Carlo_Step(float &ang_var, gsl_rng **r) {
  ...
  // Copia snapshot atual
  for (int i = 0; i < Nt * 3; i++) myNi[i] = ni[i];
...
#pragma omp for simd reduction(+ : acceptance) schedule(simd : dynamic)
    for (int nt = 0; nt < Ntt; nt++) {
      ...
      // Gerar variacao angular aleatoria
  ...
  // Ajuste dinamico do passo angular
