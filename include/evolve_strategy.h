...
public:
    ...
    virtual int run(float* ni, int* pt, Parameters* params, Geometry* geometry) = 0;
...
protected:
    ...
    float energyCalculator(float* ni, int* pt, Parameters* params, Geometry* geometry);
...
public:
    int run(float* ni, int* pt, Parameters* params, Geometry* geometry) override;
    std::string getName() const override { return "Thermal Evolution"; }
    
private:
    void thermalLoop(float* ni, int* pt, Parameters* params, Geometry* geometry, 
...
private:
    void quenchLoop(float* ni, int* pt, Parameters* params, Geometry* geometry,
                   gsl_rng** rng, FILE* po_file);
};
...
private:
    void electricLoop(float* ni, int* pt, Parameters* params, Geometry* geometry,
                     gsl_rng** rng, FILE* po_file);
};
