...
int stepEvolveN::run() {
  ...
  for (int ii = params->mc.first_file; ii < params->mc.fn + params->mc.first_file; ii++) {
    ...
    // Alteração 12: Acesso a MCT aninhado em 'mc'
    ...
    // Alteração 13: Acesso a MCS aninhado em 'mc'
    for (int step = 0; step < params->mc.MCS; step++) {
      ...
      // Alteração 14: Chamada de função do namespace OrderParameters
      OrderParameters::Matrice_constructor(ni, mat_n, pt, *params);
      
      // Alteração 15: Chamada de função do namespace OrderParameters
      sTemp = OrderParameters::Eigen_value_evaluation(mat_n, vec_n);
      S1 += sTemp;
      S2 += sTemp * sTemp;
    }
    
    // Alteração 16, 17: Acesso a MCS aninhado em 'mc'
    E /= params->mc.MCS;
    E2 /= params->mc.MCS;
    S1 /= params->mc.MCS;
    S2 /= params->mc.MCS;
    ...
    // Alteração 18: Chamada de função do namespace IO
