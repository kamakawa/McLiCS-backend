...
electricEvolveN::electricEvolveN(std::unique_ptr<float[]>& ni, std::unique_ptr<int[]>& ppt, Parameters *params)
...
int electricEvolveN::run() {
  ...
  fname = "po.dat";
  int sign = -params->elecdE / std::fabs(params->elecdE);
  ...
  for (params->elecE = params->elecEi; 
    ...
    S1 /= params->MCS;
    S2 /= params->MCS;
    fname = "director_field_" + std::to_string((int)(100 * (params->elecE + 1e-7))) + ".csv";
