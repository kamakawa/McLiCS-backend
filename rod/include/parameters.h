...
/*struct Parameters {
  ...
  std::map<int, float> theta_s; //Ângulo de ancoragem
  std::map<int, float> phi_s; //Ângulo de ancoragem
  std::map<int, float> W; //Força de ancoragem
  std::map<int, std::string> anchoring_type;  //Tipo de ancoragem
...
struct Parameters {
  ...
  //(Variáveis de Grade e Fronteira)
  ...
  //(Variáveis de Potencial e Temperatura)
  ...
  //(Variáveis do Método de Monte Carlo)
  struct MCParams {
    char evol[20] = "thermal";
    int MCS = 1e2;
    int MCT = 1e2;
  ...
  //(Variáveis de Condição Inicial e Elásticas)
  struct ICParams {
    char ic[50] = "random";
    char ic_file[100];
    float phi_0 = 0;
  ...
  //(Variáveis de Vizinhança/Escala)
